...
/* Hide unnecessary streamlit parts */
...
[data-testid="stTextInput"] input {
    background: #141614 !important;
    color: #ECE9E1 !important;
    border: 1px solid #2D302C !important;
    ...
    with R:
        ...
        if uploaded and run:
            with st.spinner("Menganalisis gambar..."):
                ...
                res, t = infer(model, bgr, conf_thresh, img_size)
                ann = cv2.cvtColor(annotate(bgr, res), cv2.COLOR_BGR2RGB)
                dets = parse(res)
    ...
    with R2:
        ...
        if cam and cam_run:
            with st.spinner("Menganalisis foto..."):
                ...
                res2, t2 = infer(model, bgr2, conf_thresh, img_size)
                ann2 = cv2.cvtColor(annotate(bgr2, res2), cv2.COLOR_BGR2RGB)
                dets2 = parse(res2)
